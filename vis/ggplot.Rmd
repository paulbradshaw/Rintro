...
## Bubble charts: changing the size of dots based on a variable 
...
plot(g)
```
## Creating a circle around key points using `geom_encircle()`

We can also highlight points with a circle using `geom_encircle()`. First we need to install and active `ggalt`.

The `geom_encircle` function needs to be told the x and y variabes again, and the dataset of the subset you want to highlight - `data=schooldata.outliers`, the `color` and `size` (thickness) can be specified, and the `expand=` parameter specifies how much it spaces out.
...
g <- ggplot(schooldata, aes(x=Full.Time.Posts.Vacant...., y=Full.Time.Temporarily.Filled.Posts....)) + 
  ...
  scale_color_manual(name="Academy", 
                     ...
                     values = c("TRUE" = "red","FALSE" = "blue"))  +
  #--------------here's the NEW bit-----------
  geom_encircle(aes(x=Full.Time.Posts.Vacant...., y=Full.Time.Temporarily.Filled.Posts....),
                data=schooldata.outliers, 
                color="red",
                size=2,
                expand=0.05
                )
...
## Counting to account for overlapping points

In our chart there may be more than one school with the same stats, but they will sit on top of each other. 
...
#The code to draw - stripped back a little to remove the placement of the legend.
g <- ggplot(schooldata, aes(x=Full.Time.Posts.Vacant...., y=Full.Time.Temporarily.Filled.Posts....)) +
  # Here is where geom_point is replaced with geom_count ------------------------------------------------------
...
You can also use `geom_jitter()` instead of `geom_point()` to jitter the points within a specified range of pixels around the actual point, so that they are more visible. [More here](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html#Jitter%20Plot)

## Next steps: other charts

That's the end of exploring scatterplots. I've looked at other types of charts in a separate R notebook.
