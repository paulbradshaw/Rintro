...
Again, chances are you'll need to generate those somehow. For example, by choosing 3-5 colours to represent different 'buckets' of values (lowest quartile, highest quartile, and so on) and assigning them using a formula like `IF`
...
plot(n, col = n$color, lwd = 1, main = "This is the map heading", sub = "Black = some random colour\nPink = another random colour")
```
